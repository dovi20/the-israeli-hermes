...
COMMAND_REGISTRY: list[CommandDef] = [
    # Session
    CommandDef("new", "התחל סשן חדש (session ID והיסטוריה חדשים)", "Session",
               aliases=("reset",)),
    CommandDef("clear", "נקה מסך והתחל סשן חדש", "Session",
               cli_only=True),
    CommandDef("redraw", "אילוץ ציור מחדש של ה-UI (משחזר מהיסחפות טרמינל)", "Session",
               cli_only=True),
    CommandDef("history", "הצג היסטוריית שיחה", "Session",
               cli_only=True),
    CommandDef("save", "שמור את השיחה הנוכחית", "Session",
               cli_only=True),
    CommandDef("retry", "נסה שוב את ההודעה האחרונה (שלח מחדש לסוכן)", "Session"),
    CommandDef("undo", "הסר את חילופי ההודעות האחרונים (משתמש/עוזר)", "Session"),
    CommandDef("title", "הגדר כותרת לסשן הנוכחי", "Session",
               args_hint="[name]"),
    CommandDef("branch", "צור הסתעפות מהסשן הנוכחי (חקור נתיב אחר)", "Session",
               aliases=("fork",), args_hint="[name]"),
    CommandDef("compress", "דחוס ידנית את קונטקסט השיחה", "Session",
               args_hint="[focus topic]"),
    CommandDef("rollback", "הצג או שחזר checkpoints של מערכת הקבצים", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "צור או שחזר snapshots של תצורה/מצב Hermes", "Session",
               cli_only=True, aliases=("snap",), args_hint="[create|restore <id>|prune]"),
    CommandDef("stop", "סגור את כל התהליכים הרצים ברקע", "Session"),
    CommandDef("approve", "אשר פקודה מסוכנת ממתינה", "Session",
               gateway_only=True, args_hint="[session|always]"),
    CommandDef("deny", "דחה פקודה מסוכנת ממתינה", "Session",
               gateway_only=True),
    CommandDef("background", "הרץ פרומפט ברקע", "Session",
               aliases=("bg", "btw"), args_hint="<prompt>"),
    CommandDef("agents", "הצג סוכנים פעילים ומשימות רצות", "Session",
               aliases=("tasks",)),
    CommandDef("queue", "הוסף פרומפט לתור (לא מפריע לתור הנוכחי)", "Session",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "הזרק הודעה אחרי קריאת הכלי הבאה בלי להפסיק", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "הגדר מטרה ש-Hermes יעבוד עליה לאורך תורים עד להשלמה", "Session",
               args_hint="[text | pause | resume | clear | status]"),
    CommandDef("status", "הצג מידע על הסשן", "Session"),
    CommandDef("profile", "הצג שם פרופיל פעיל ותיקיית בית", "Info"),
    CommandDef("sethome", "קבע צ'אט זה כערוץ הבית", "Session",
               gateway_only=True, aliases=("set-home",)),
    CommandDef("resume", "המשך סשן עם שם קיים", "Session",
               args_hint="[name]"),

    # Configuration
    CommandDef("config", "הצג תצורה נוכחית", "Configuration",
               cli_only=True),
    CommandDef("model", "החלף מודל לסשן זה", "Configuration",
               aliases=("provider",), args_hint="[model] [--provider name] [--global]"),
    CommandDef("gquota", "הצג שימוש במכסת Google Gemini Code Assist", "Info",
               cli_only=True),

    CommandDef("personality", "קבע אישיות מוגדרת מראש", "Configuration",
               args_hint="[name]"),
    CommandDef("statusbar", "הפעל/כבה את שורת הסטטוס של קונטקסט/מודל", "Configuration",
               cli_only=True, aliases=("sb",)),
    CommandDef("verbose", "החלף תצוגת התקדמות כלים: off -> new -> all -> verbose",
               "Configuration", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("footer", "הפעל/כבה footer של מטא-נתוני runtime על תגובות סופיות",
               "Configuration", args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("yolo", "הפעל/כבה מצב YOLO (דלג על כל אישורי פקודות מסוכנות)",
               "Configuration"),
    CommandDef("reasoning", "נהל מאמץ חשיבה ותצוגתה", "Configuration",
               args_hint="[level|show|hide]",
               subcommands=("none", "minimal", "low", "medium", "high", "xhigh", "show", "hide", "on", "off")),
    CommandDef("fast", "הפעל/כבה מצב מהיר — OpenAI Priority / Anthropic Fast (Normal/Fast)", "Configuration",
               args_hint="[normal|fast|status]",
               subcommands=("normal", "fast", "status", "on", "off")),
    CommandDef("skin", "הצג או שנה את ערכת הנושא/skin", "Configuration",
               cli_only=True, args_hint="[name]"),
    CommandDef("indicator", "בחר סגנון של מחוון עבודה ב-TUI", "Configuration",
               cli_only=True, args_hint="[kaomoji|emoji|unicode|ascii]",
               subcommands=("kaomoji", "emoji", "unicode", "ascii")),
    CommandDef("voice", "הפעל/כבה מצב קול", "Configuration",
               args_hint="[on|off|tts|status]", subcommands=("on", "off", "tts", "status")),
    CommandDef("busy", "שלוט במה ש-Enter עושה בזמן ש-Hermes עובד", "Configuration",
               cli_only=True, args_hint="[queue|steer|interrupt|status]",
               subcommands=("queue", "steer", "interrupt", "status")),

    # Tools & Skills
    CommandDef("tools", "נהל כלים: /tools [list|disable|enable] [name...]", "Tools & Skills",
               args_hint="[list|disable|enable] [name...]", cli_only=True),
    CommandDef("toolsets", "הצג ערכות כלים זמינות", "Tools & Skills",
               cli_only=True),
    CommandDef("skills", "חפש, התקן, בדוק או נהל מיומנויות",
               "Tools & Skills", cli_only=True,
               subcommands=("search", "browse", "inspect", "install")),
    CommandDef("cron", "נהל משימות מתוזמנות", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("curator", "תחזוקת skills ברקע (status, run, pin, archive)",
               "Tools & Skills", args_hint="[subcommand]",
               subcommands=("status", "run", "pause", "resume", "pin", "unpin", "restore")),
    CommandDef("kanban", "לוח שיתוף-פרופילים (משימות, קישורים, תגובות)",
               "Tools & Skills", args_hint="[subcommand]",
    ...
    CommandDef("reload", "טען מחדש משתני .env לסשן הרץ", "Tools & Skills",
               cli_only=True),
    CommandDef("reload-mcp", "טען מחדש שרתי MCP מהתצורה", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "סרוק מחדש ~/.hermes/skills/ ל-skills חדשים או מוסרים",
               "Tools & Skills", aliases=("reload_skills",)),
    CommandDef("browser", "חבר כלי דפדפן ל-Chrome חי דרך CDP", "Tools & Skills",
               cli_only=True, args_hint="[connect|disconnect|status]",
               subcommands=("connect", "disconnect", "status")),
    CommandDef("plugins", "הצג תוספים מותקנים וסטטוס",
               "Tools & Skills", cli_only=True),

    # Info
    CommandDef("commands", "עיין בכל הפקודות וה-skills (עם עימוד)", "Info",
               gateway_only=True, args_hint="[page]"),
    CommandDef("help", "הצג פקודות זמינות", "Info"),
    CommandDef("restart", "אתחל את ה-gateway באופן מבוקר אחרי סיום ריצות פעילות", "Session",
               gateway_only=True),
    CommandDef("usage", "הצג שימוש בטוקנים ו-rate limits לסשן הנוכחי", "Info"),
    CommandDef("insights", "הצג תובנות שימוש ואנליטיקה", "Info",
               args_hint="[days]"),
    CommandDef("platforms", "הצג סטטוס gateway/פלטפורמות מסרים", "Info",
               cli_only=True, aliases=("gateway",)),
    CommandDef("copy", "העתק את התגובה האחרונה של העוזר ללוח", "Info",
               cli_only=True, args_hint="[number]"),
    CommandDef("paste", "צרף תמונה מהלוח", "Info",
               cli_only=True),
    CommandDef("image", "צרף קובץ תמונה מקומי לפרומפט הבא", "Info",
               cli_only=True, args_hint="<path>"),
    CommandDef("update", "עדכן את Hermes Agent לגרסה האחרונה", "Info",
               gateway_only=True),
    CommandDef("debug", "העלה דוח דיבאג (מידע מערכת + לוגים) וקבל לינקים לשיתוף", "Info"),

    # Exit
    CommandDef("quit", "צא מה-CLI", "Exit",
